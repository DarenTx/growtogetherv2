...
Use this rule to decide where a new component lives:

| Location                         | Rule                                                                     |
| -------------------------------- | ------------------------------------------------------------------------ |
| `features/[feature]/components/` | Used **only within one feature** (sub-components, presentational pieces) |
| `features/[feature]/`            | Routed **page components** for that feature                              |
| `shared/components/`             | Consumed by **two or more distinct features**                            |
...
features/
  admin/
    historical-data/          ← routed page for admin
    market-data/              ← routed page for admin
    profiles/                 ← routed page for admin
  dashboard/
    components/
      classic-scorecard/      ← dashboard sub-component only
      growth-grid/            ← dashboard sub-component only
      select-year/            ← dashboard sub-component only
    monthly-growth-entry/     ← routed sub-page of dashboard
    dashboard.component.*
  audit-log/                  ← routed page (top-level feature)
  auth/
    login/ callback/ ...      ← routed pages for auth

shared/
  components/
    footer/                   ← used in app shell (app.ts)
    person-selector/          ← will be reused across admin + other features
    trend-label/              ← will be reused across multiple features
```
...
1. Is it a routed page? → `features/[feature]/` (no `components/` subfolder)
2. Is it (or will it likely be) used in more than one feature? → `shared/components/`
3. Otherwise → `features/[feature]/components/`

When in doubt, prefer feature-scoped placement. It is easier to promote a component to `shared/` later than to demote it.
